...
function M.build_chest_border_picture(positions)
  local all_layers = {}
  for _, position in ipairs(positions) do
    for _, layer in ipairs(chest_at(position[1], position[2])) do
...
function M.build_directional_border_animation()
  local horizontal = M.build_chest_border_picture(M.h_positions)
  local vertical   = M.build_chest_border_picture(M.v_positions)
  return {
    north = horizontal,
    south = horizontal,
    east  = vertical,
